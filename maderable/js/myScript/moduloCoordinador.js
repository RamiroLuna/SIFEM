    $(document).on('ready', function() {
        ...
        loadTablaTermino('getPanel', amplify.store('dataLoggingProbosque').id);
    ...
    function reloadPage(){
       window.location.assign('/SIFEM/maderable/moduloCoordinador.html');
    }
    ...
    function ctrlCheckBoxAbogado(e){
        ...
        $('#btn-confirmacion-Ok').attr('fila',$(e).attr('fila'));
        $('#btn-confirmacion-Ok').attr('tabla',$(e).attr('tabla'));
                ...
                if (!isEmptyValidateText($('#fechaConclusion').val())) {
                    if (IsDate($('#fechaConclusion').val())) {
                         ...
                         modalConfirmacion('¿Está seguro de asignar <u>'+selectTextoAbogado('#divProceso input','#divConcluido input')+'</u> como estatus ?', 
                          arrayElementSeletedName('#divProceso input','#divConcluido input'),2);
                    } else {
    ...
    function showModalStatus(e) {
        ...
        $('#btnAceptOficial,#btnAceptSemaforo,#btnAceptAbogado').attr({'expediente':$(e).attr('expediente'), 'fila':$(e).attr('fila'),'tabla':$(e).attr('tabla')});
        ...
        for (var i = 0; i < $(e).attr('estatus').split(',').length; i++) {
            ...
            switch ($(e).attr('estatus').split(',')[i]) {
                ...
                //Estatus del abogado
                case '10':
                    $("#radioProceso").attr('checked', true);
    ...
    function sendStatus(e){
        switch($(e).attr('option')){
            case '1':
              sendStatusAjax('setEstatus', amplify.store('dataLoggingProbosque').id , $(e).attr('expediente'), $(e).attr('estatus').trim(), 
                          $(e).attr('fila').trim(), $(e).attr('tabla').trim());
            break;
            case '2':  
               if(!isEmptyValidateText($('#addObservaciones').val())){
                 sendStatusAjaxAbogado('changeEstatus', amplify.store('dataLoggingProbosque').id, 
                                            ...
                                            $(e).attr('multa'), $(e).attr('decomiso'), $(e).attr('fila'), $(e).attr('tabla'));
               }else{
...
function drawTablePlugin(tabla) {
    ...
    function isProcess(array) {
        var b= -1;
        if(!isEmptyValidateText(array)){

          for (var i = 0; i < array.split(',').length; i++) {
              if(parseInt(array.split(',')[i]) <= 19){
                  b = parseInt(array.split(',')[i]);
                  break;
              }else if( (parseInt(array.split(',')[i]) >= 20 && parseInt(array.split(',')[i]) <= 29) || parseInt(array.split(',')[i]) == 36 ){
                   b = -2;
              }
          }
        }
    ...
    function textoEstatus(number) {
        ...
        switch (number) {
            ...
            case 9:
                ...
                break;
            case 10:
                texto = 'En termino de 5 días';
                break;
            case 11:
                texto = 'Primer dictamen';
                break;
            case 12:
                texto = 'Para emplazar';
                break;
            case 13:
                texto = 'Emplazado 15 días';
                break;
            case 14:
                texto = 'Análisis de documentación 2do dictamen';
                break;
            case 15:
                texto = 'Alegatos';
                break;
            case 16:
                texto = 'Para emitir resolución';
                break;
            case 17:
                texto = 'Resolución administrativa';
                break;
            case 18:
                texto = 'Para notificar';
                break;
            case 19:
                texto = 'Resolución notificada';
                break;
        }
        return texto;
    }
    ...
    function printTable(arreglo, id, numAuxiliar) {
        $('#' + id).empty();
        var cuerpoTabla = document.getElementById(id);
        if (arreglo.length > 0) {
            var fila = cuerpoTabla.insertRow();
            fila.style.display = 'none';
            fila.setAttribute('id', 'auxTmp' + numAuxiliar);
            var celda = fila.insertCell(-1);
            celda.colSpan = 4;
            celda.setAttribute('class', 'text-center');
            celda.innerHTML = 'No se encontráron coincidencias';
            var Numerofila=0;
            for (var aux in arreglo) {
                ...
                if (id == 'tbodyTermino') {
                    fila.setAttribute('id','rentermino'+Numerofila);
                    ...
                    if(isProcess(estatus)){
                      fila.insertCell(-1).innerHTML = textoEstatus(isProcess(estatus));
                    }else{
                      fila.insertCell(-1).innerHTML = '<button folio="'+folioTmp+'"  id="btntermino'+Numerofila+'" fila="'+Numerofila+'" tabla="termino" abo="'+abogado+'" obsPublic="'+observacionesPublicas+'" sem="'+semaforo+'" multa="'+mul+'" decomiso="'+deco+'" fecha_con="'+fech+'"  expediente="'+noExpediente+'" estatus="'+estatus+'" onclick="showModalStatus(this)" class="btn btn-success btn-block">Asignar</button>';
                    }
                    
                } else if (id == 'tbodyProceso') {
                    fila.setAttribute('id','renproceso'+Numerofila);
                    celdaUsuario.setAttribute('id','proceso'+Numerofila+'2');
                     celdaExpediente.setAttribute('id','proceso'+Numerofila+'1');
                    fila.insertCell(-1).innerHTML = '<button expediente="'+noExpediente+'" id="btnprocesoObser'+Numerofila+'" onclick="showModalObservaciones(this)" class="btn btn-default btn-block">Ver</button>';
                     if(isProcess(estatus)){
                      fila.insertCell(-1).innerHTML = textoEstatus(isProcess(estatus));
                    }else{
                    fila.insertCell(-1).innerHTML = ' <button folio="'+folioTmp+'"  id="btnproceso'+Numerofila+'" fila="'+Numerofila+'" tabla="proceso" abo="'+abogado+'"  obsPublic="'+observacionesPublicas+'" sem="'+semaforo+'" multa="'+mul+'" decomiso="'+deco+'" fecha_con="'+fech+'"  expediente="'+noExpediente+'" estatus="'+estatus+'" onclick="showModalStatus(this)" class="btn btn-custom btn-block">Asignar</button>';
                     }
                } else if (id == 'tbodyRechazo') {
                    fila.setAttribute('id','renrechazo'+Numerofila);
                    celdaUsuario.setAttribute('id','rechazo'+Numerofila+'2');
                     celdaExpediente.setAttribute('id','rechazo'+Numerofila+'1');
                    fila.insertCell(-1).innerHTML = '<button expediente="'+noExpediente+'" id="btnrechazoObser'+Numerofila+'" onclick="showModalObservaciones(this)" class="btn btn-default btn-block">Ver</button>';
                       if(isProcess(estatus)){
                      fila.insertCell(-1).innerHTML = textoEstatus(isProcess(estatus));
                    }else{
                    fila.insertCell(-1).innerHTML = '<button folio="'+folioTmp+'" id="btnrechazo'+Numerofila+'"  fila="'+Numerofila+'" tabla="rechazo" abo="'+abogado+'"  obsPublic="'+observacionesPublicas+'" sem="'+semaforo+'" multa="'+mul+'" decomiso="'+deco+'" fecha_con="'+fech+'" expediente="'+noExpediente+'"  estatus="'+estatus+'" onclick="showModalStatus(this)" class="btn btn-warning btn-block">Asignar</button>';
                  }
                } else if (id == 'tbodyVencido') {
                    fila.setAttribute('id','renvencido'+Numerofila);
                    celdaUsuario.setAttribute('id','vencido'+Numerofila+'2');
                    celdaExpediente.setAttribute('id','vencido'+Numerofila+'1');
                    fila.insertCell(-1).innerHTML = '<button expediente="'+noExpediente+'" id="btnvencidoObser'+Numerofila+'" onclick="showModalObservaciones(this)" class="btn btn-default btn-block">Ver</button>';
                      if(isProcess(estatus)){
                      fila.insertCell(-1).innerHTML = textoEstatus(isProcess(estatus));
                    }else{
                    fila.insertCell(-1).innerHTML = '<button folio="'+folioTmp+'"  id="btnvencido'+Numerofila+'" fila="'+Numerofila+'" tabla="vencido" abo="'+abogado+'" obsPublic="'+observacionesPublicas+'" sem="'+semaforo+'" multa="'+mul+'" decomiso="'+deco+'" fecha_con="'+fech+'"  expediente="'+noExpediente+'" estatus="'+estatus+'" onclick="showModalStatus(this)" class="btn btn-danger btn-block">Asignar</button>';
                    }
                }
    ...
    /**********************************************************************************
                                 Peticiones AJAX               
    ***********************************************************************************/
    function loadTablaTermino(action, user) {
        var r = {
            success: function(json) {
              console.log(json);
                ...
                if (b) {
                    ...
                    printTable(json.data.EnTermino, 'tbodyTermino', 1);
                    printTable(json.data.EnProceso, 'tbodyProceso', 2);
                    printTable(json.data.Retraso, 'tbodyRechazo', 3);
                    printTable(json.data.Vencido, 'tbodyVencido', 4);
...
function sendStatusAjax(action, user, expediente, estatus , fila , tabla) {
    var r = {
        success: function(json) {
            var b = false;
            var mensaje = '';
            ...
            if (b) {
                //Resetea los formularios del abogado
                 $("#formularioAbogado input").each(function() {
                    if ($(this).attr('type') == 'checkbox') {
                ...
                //Fin resetea y asigna estatus

                $('#btn'+tabla+fila).attr({'estatus':estatus,'multa':'','fecha_con':'','decomiso':''});
...
function sendStatusAjaxAbogado(action, user, expediente, estatus, observaciones, fecha_con, multa, decomiso, fila, tabla) {
    var f = new Date();
    ...
    var r = {
        success: function(json) {
            ...
            if (b) {
                $('#btn' + tabla + fila).attr({ 'estatus': estatus, 'decomiso': decomiso, 'fecha_con': fecha_con, 'multa': multa });
...
function sendStatusAjaxcoordinador(action, user, expediente, estatus, observaciones, fila, tabla) {
    var r = {
        success: function(json) {
            console.log(json);
            var b = false;
            ...
            if (b) {
                $('#btn' + tabla + fila).attr({ 'sem': estatus,'obspublic': observaciones });
                $('#ren'+tabla+fila).remove();
                $('#modalAsignarEstatus').modal('hide');
                $('#confirmacion').modal('hide');
                showSucessCalif('Se ha asignado estatus programa madera legal México', 'Aceptar');
            } else {
   ...
   var r = {
        success: function(json) {
            ...
            if (b) {
               ...
               $('#textoAbogadoAsignado').html(textoNombre);
   ...
   var r = {
        success: function(json) {
            ...
            if (b) {  
               ...
               $('#textoNumeroExpediente').html(expedienteNuevo);
               $('#modalOption').modal('hide');
            } else {
...
function ctrlCheckBox(c) {
        var count = 0;
        $('#btn-confirmacion-Ok').attr('fila','');
        $('#btn-confirmacion-Ok').attr('tabla','');
        $('#btn-confirmacion-Ok').attr('expediente','');
        ...
        if (count == 1) {
            $('#btn-confirmacion-Ok').attr('fila',$(c).attr('fila'));
            $('#btn-confirmacion-Ok').attr('tabla',$(c).attr('tabla'));
            $('#btn-confirmacion-Ok').attr('expediente',$(c).attr('expediente'));
            $('#addObservaciones').hide();
            if($('#check1').is(':checked')){
                modalConfirmacion('¿Está seguro de asignar <u>'+$('#check1').val()+'</u> como estatus ?', 1 , 1);
            }else{
                  modalConfirmacion('¿Está seguro de asignar <u>'+selectCheckTexto("#formularioOficialia input")+'</u> como estatus ?',
                                       getStatusSelected(arrayElementSeleted("#formularioOficialia input")),1);              
            }
        }
}
...
function ctrlSemaforo(c){
  ...
  if(contador == 1){
    modalConfirmacion('¿Está seguro de asignar '+arrayElementSeletedTextoSemaforo('#formularioSemaforo input')+' como estatus ?',
                      arrayElementSelectedNameSem('#formularioSemaforo input'),3);
  }else{
