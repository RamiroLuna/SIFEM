    $(document).on('ready', function() {
         ...
         loadTableImpugnacion('getImpugnaciones', amplify.store('dataLoggingProbosque').id);
    ...
    function reloadPage(){
       window.location.assign('/SIFEM/maderable/impugnaciones.html');
    }
    ...
    function showModalStatus(e) {
        ...
        $('#btnAceptOficial,#btnAceptSemaforo,#btnAceptAbogado').attr('expediente', $(e).attr('expediente'));
    ...
    function sendStatus(e){
        switch($(e).attr('option')){
            case '1':
              sendStatusAjax('setEstatus', amplify.store('dataLoggingProbosque').id , $(e).attr('expediente'), $(e).attr('estatus') );
            break;
            case '2':  
    ...
    var modalSucessCal = function(mensaje, btnText) {
        var modal =
            ...
            '<br>' +
            '<button type="button" class="btn btn-success" data-dismiss="modal" id="btn-addOk" onclick="reloadPage(this)">' + btnText + '</button>' +
            '</center>' +
            '</div>' +
            '</div>' +
            '</div>' +
...
function getStatusSelected(cadena){
   ...
   return arrTmp;
     ...
     function printTable(arreglo, id) {
        ...
        if (arreglo.length > 0) {
              ...
              for (var aux in arreglo) {
                ...
                var abogado='';
                Numerofila++;
                fila.setAttribute('id','tablaRenglon'+Numerofila);
                ...
                fila.insertCell(-1).innerHTML = '<button folio="'+folioTmp+'"  id="btntermino'+Numerofila+'" fila="'+Numerofila+'" tabla="termino" abo="'+abogado+'" obsPublic="'+observacionesPublicas+'" sem="'+semaforo+'" multa="'+mul+'" decomiso="'+deco+'" fecha_con="'+fech+'"  expediente="'+noExpediente+'" estatus="'+estatus+'" onclick="showModalStatus(this)" class="btn btn-default btn-block">Se repone</button>';
                fila.insertCell(-1).innerHTML = '<button expediente="'+noExpediente+'" id="btnterminoRepo'+Numerofila+'" fila="'+Numerofila+'" onclick="quitarImpugnacion(this)" class="btn btn-default btn-block">Se confirma</button>';
              ...
              var fila = cuerpoTabla.insertRow();
                       for (var i = 0; i < 5; i++) {
                           if (i == 2) {
                            ...
                            celda.setAttribute('with','96%');
                            celda.innerHTML = '<center> No hay registros para mostrar</center>';
                           } else {

                            fila.insertCell(-1).setAttribute('with','1%');;
                           }
                }
        }

        drawTablePlugin('tableofImp');
    }
    ...
    function quitarImpugnacion(e){
        modalQuitarImpugnacion(e);
    }
...
function sendStatusAjax(action, user, expediente, estatus ) {
    var r = {
        success: function(json) {
            var b = false;
            var mensaje = '';
...
function sendStatusAjaxAbogado(action, user, expediente, estatus, observaciones, fecha_con , multa , decomiso) {
    var f = new Date();
...
function sendStatusAjaxcoordinador(action, user, expediente, estatus, observaciones) {
    var r = {
        success: function(json) {
            console.log(json);
            var b = false;
   ...
   var r = {
        success: function(json) {
            ...
            if (b) {
               ...
               $('#tableofImp').DataTable().cell( parseInt(fila)-1, 1 ).data(textoNombre).draw();
   ...
   var r = {
        success: function(json) {
            ...
            if (b) {  
               ...
               $('#btn'+tabla+'Repo'+fila).attr('expediente',expedienteNuevo);
               ...
               $('#tableofImp').DataTable().cell( parseInt(fila)-1, 1 ).data(expedienteNuevo).draw();
               $('#modalOption').modal('hide');
            } else {
...
function loadTableImpugnacion(action, user) {
        var r = {
            success: function(json) {
              console.log(json);
                var b = false;
                ...
                if (b) {
                    ...
                    printTable(json.data, 'tbodyImpugnacion');
  ...
  function quitarImpugnacionAJAX(action, user , expediente , fila, observacion,fecha){
       var r = {
            success: function(json) {
              console.log(json);
                var b = false;
                ...
                if (b) {
                     ...
                     $('#tableofImp').DataTable().row('#tablaRenglon'+fila).remove().draw( false );
                     $('#modalOption').modal('hide');
                     showSucessCalif('Se quito impugnación al expediente '+expediente, 'Aceptar');

                } else {
                    showErrorCalif("No se pudo quitar impugnación", "Intentar");
                }
            },
            beforeSend: function(xhr) {},
            error: function(XMLHttpRequest, textStatus, errorThrown) {
        ...
        r = $.extend(r, urlServiceCoordinador);
        r.data = { action: action, user: user , noExpediente: expediente, observaciones:observacion,fecha:fecha};
        $.ajax(r);
  }
...
function modalQuitarImpugnacion(parametro) {

    var expediente='';
    var fila='';
    if(typeof $(parametro).attr('expediente') == 'undefined'){
        expediente='';
    }else{
        expediente=$(parametro).attr('expediente');
    }

    if(typeof $(parametro).attr('fila') == 'undefined'){
        fila='';
    }else{
        fila=$(parametro).attr('fila');
    }
    ...
    $('#bodyOption').html(
        '<div class="col-md-12 col-sm-12 col-xs-12">' +
        '<center><b>¿Está seguro de quitar la impugnación al expediente '+expediente+' ?</b></center><br>'+
        '<textarea class="form-control" rows="5" id="observacionConfirmacion" placeholder="Añadir observación"></textarea>'+
        '</div>'
    );
    ...
    $('#footerOption').html(
        ...
        '</div>' +
        '<div class="col-md-4 col-sm-12 col-xs-12">' +
        '<button type="button"  expediente="'+expediente+'" fila="'+fila+'" onclick="solicitaQuitarImpugnacion(this)" class="btn btn-default">Confirmar</button>' +
        '</div>' +
        '</div>'
    );
...
function solicitaQuitarImpugnacion(e){
    var f = new Date();
    var fecha_observacion = f.getDate() + "/" + (f.getMonth() + 1) + "/" + f.getFullYear();
    quitarImpugnacionAJAX('reposicion', amplify.store('dataLoggingProbosque').id ,$(e).attr('expediente'), $(e).attr('fila'),$('#observacionConfirmacion').val().trim(),fecha_observacion);
}
